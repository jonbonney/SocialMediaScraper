...
def parse_new(driver, cards, time_start):
...
def parse_old(driver, time_start):
    print('checking for older page design...')
    card = driver.find_element(by=By.CSS_SELECTOR, value="div[class='_4-u2 _3xaf _7jo_ _4-u8']")
    # On these older pages, we cannot click the "See all" button on the Page Transparency card without being logged in
...
def exit_login_popup(driver, time_start):
    # # # This method was to exit a dialog that appears asking to log into facebook
...
def main():
    ...
    # Request the webpage
    driver.get('https://www.facebook.com/8x8Inc')
    ...
    if cards:
        parse_new(driver, cards, time_start)
    # If no cards were found, this page might still be on the old UI design
    else:
        parse_old(driver, time_start)
